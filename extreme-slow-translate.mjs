...
async function processLanguage(langCode, index, total) {
  ...
  for (let i = 0; i < keys.length; i++) {
    ...
    // EXTREME DELAY - 5 seconds per string
    await sleep(5000);
  }
  ...
  // EXTREME wait between languages - 30 seconds
  if (index < total) {
    console.log(`   ⏳ Waiting 30s before next language...`);
    await sleep(30000);
  }
}

async function main() {
  console.log('🐌 EXTREME SLOW TRANSLATION\n');
  console.log('Strategy: 5s per string, 30s between languages');
  console.log('WILL COMPLETE: YES ✅');
  console.log('Speed: SLOW but GUARANTEED ✅\n');
