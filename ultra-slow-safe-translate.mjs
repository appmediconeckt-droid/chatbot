...
async function translate(text, targetLang) {
  try {
    ...
    const res = await axios.post(API, { text, to: shortLang, from: 'en' }, { timeout: 15000 });
...
async function processLanguage(langCode, index, total) {
  ...
  for (let i = 0; i < keys.length; i++) {
    ...
    // VERY LONG delay between individual translations (500ms)
    await sleep(500);
  }
...
async function main() {
  console.log('🐌 ULTRA-SLOW SAFE TRANSLATION - Respects API Limits\n');
  console.log('⏱️  Delays: 500ms between keys, 10s between languages\n');

  const allLangs = Object.keys(langMap);

  for (let i = 0; i < allLangs.length; i++) {
    await processLanguage(allLangs[i], i + 1, allLangs.length);
    console.log(`   ⏳ Waiting 10 seconds before next language...`);
    await sleep(10000); // 10 second wait between languages
  }

  console.log('\n🎉 ALL LANGUAGES TRANSLATED SAFELY!');
}
