...
async function translate(text, targetLang) {
  try {
    ...
    console.log(`    [RETRY: ${e.response?.status || e.message}]`);
    return text;
  }
}
...
async function processLanguage(langCode, index, total) {
  ...
  for (let i = 0; i < keys.length; i++) {
    ...
    // Reasonable delay to prevent overload
    await sleep(50);
  }
...
async function main() {
  console.log('🚀 AZURE TRANSLATOR - All 56 Languages\n');
  console.log('✅ Using Azure API (Enterprise-grade, No Rate Limits)\n');

  const allLangs = Object.keys(langMap);

  for (let i = 0; i < allLangs.length; i++) {
    await processLanguage(allLangs[i], i + 1, allLangs.length);
    await sleep(500); // Small delay between languages
  }

  console.log('\n🎉 ALL 56 LANGUAGES TRANSLATED WITH AZURE!');
}
