// ratingService.js — web client for the eligibility-driven rating flow.
// ---------------------------------------------------------------------------
...
export async function checkEligibility() {
  try {
    const { data } = await axiosInstance.get("/api/ratings/check-eligibility");
...
export async function submitRating({ counselorId, rating, review = "" }) {
  ...
  const { data } = await axiosInstance.post("/api/ratings/submit", {
    counselorId,
    rating,
    review: review?.trim() || "",
  });
...
export async function remindLater(counselorId) {
  ...
  try {
    await axiosInstance.post("/api/ratings/remind-later", { counselorId });
  } catch (e) {
...
export async function neverAskAgain(counselorId) {
  if (!counselorId) return;
  try {
    await axiosInstance.post("/api/ratings/never-ask-again", { counselorId });
  } catch (e) {
...
export async function getCounselorRating(counselorId) {
  ...
  try {
    const { data } = await axiosInstance.get(
      `/api/counselors/${counselorId}/rating`
    );
    return data;
  } catch (e) {
