...
const CallModal = ({ isOpen, onClose, callType, callerName, callerImage, callData, onAccept, onEnd, onAcceptCall, onRejectCall, onJoinCall }) => {
  ...
  const handleAccept = async () => {
    ...
    if (onAcceptCall && callData) {
      try {
        // 1. Accept the call
        const acceptResult = await onAcceptCall(callData.callId);
        if (!acceptResult) {
        ...
        // 2. Automatically join the call
        if (onJoinCall) {
          const joinResult = await onJoinCall(callData.callId);
          if (joinResult) {
            setCallStatus('connected');
            if (onAccept) onAccept();
          } else {
            console.error('Failed to join call after accept');
            setCallStatus('ended');
            setTimeout(() => {
              if (onEnd) onEnd();
              onClose();
        ...
        console.error('Error in accept+join flow:', error);
        setCallStatus('ended');
      }
    }
    setIsAccepting(false);
  ...
  const handleJoin = async () => {
    ...
    if (onJoinCall && callData) {
      try {
        ...
        if (result) setCallStatus('connected');
      } catch (error) {
                ...
                {callType === 'video' && (
                  <>
                    <button className="call-btn icon-btn" onClick={() => {}}>
                      <FaMicrophone /><span>Mute</span>
                    </button>
                    <button className="call-btn icon-btn" onClick={() => {}}>
                      <FaVideoIcon /><span>Camera</span>
                    </button>
                  </>
                )}
  ...
  <div className="chat-popup-overlay">
    <div className="chat-popup">
      ...
      <div className="chat-popup-body" ref={chatBodyRef}>
        {messages.map(message => (
          <div key={message.id} className={`chat-message-wrapper ${message.sender}`}>
            {message.sender === 'ai' && <div className="chat-avatar small"><FaRobot /></div>}
            <div className="chat-bubble">{message.text}</div>
            {message.sender === 'user' && <div className="chat-avatar small"><FaUserCircle /></div>}
          </div>
        ))}
        {isLoading && (
          <div className="chat-message-wrapper ai">
            <div className="chat-avatar small"><FaRobot /></div>
      ...
      <div className="chat-popup-footer">
        <input type="text" placeholder="Type your message..." value={newMessage} onChange={(e) => setNewMessage(e.target.value)} onKeyPress={handleKeyPress} className="chat-input" />
        <button className="send-btn" onClick={sendMessage} disabled={isLoading || !newMessage.trim()}><FaPaperPlane /></button>
      </div>
    </div>
  </div>
);
...
const ChatButton = ({ onClick, unreadCount }) => (
  <button className="floating-chat-btn" onClick={onClick}>
    AI{unreadCount > 0 && <span className="unread-badge">{unreadCount}</span>}
  </button>
);
...
export default function UserDashboard() {
  ...
  const [callerInfo, setCallerInfo] = useState({
    name: '', image: null, userId: '', userName: '', callId: '', roomId: ''
  });
  ...
  // ========== CALL API FUNCTIONS (all use consistent base) ==========
  const fetchWaitingCalls = async () => {
    try {
      ...
      console.log("📡 Fetching waiting calls for user:", userId);
      const response = await axios.get(`${API_BASE_URL}/api/video/calls/waiting/${userId}`, {
        headers: { Authorization: `Bearer ${token}` }
      });
      console.log("✅ API Response:", response.data);

      // Normalize response: support { waitingCalls: [...] } or direct array
      let waitingCalls = [];
      if (response.data?.waitingCalls && Array.isArray(response.data.waitingCalls)) {
        waitingCalls = response.data.waitingCalls;
      } else if (response.data?.data && Array.isArray(response.data.data)) {
        waitingCalls = response.data.data;
      } else if (Array.isArray(response.data)) {
        waitingCalls = response.data;
      } else if (response.data?.call) {
        waitingCalls = [response.data.call];
      }
      ...
      // Find the first incoming call (status = waiting/ringing/initiated/pending/incoming)
      const incomingCall = waitingCalls.find(call => {
        const status = call.status?.toLowerCase();
        return ["waiting", "ringing", "initiated", "pending", "incoming"].includes(status);
      });

      if (!incomingCall) {
        console.log("No active incoming call found among:", waitingCalls.map(c => c.status));
        return;
      }

      console.log("🎯 Incoming call detected:", incomingCall);

      const callId = incomingCall.callId || incomingCall._id || incomingCall.id;
      if (!callId) {
        console.error("❌ Call has no ID", incomingCall);
        return;
      }

      // Extract counselor / caller info
      const counselor = incomingCall.counselor || incomingCall.initiator || incomingCall.caller || {};
      const avatar = counselor?.gender === "female" ? "👩" : counselor?.gender === "male" ? "👨" : "👤";
      const callerName = counselor?.name || counselor?.fullName || incomingCall.callerName || "Counselor";

      setCallerInfo({
        name: callerName,
        image: avatar,
        userId: counselor?.id || counselor?.userId,
        userName: callerName,
        callId: callId,
      ...
      setShowCallModal(true);   // 🔔 OPEN THE MODAL

      // Vibrate for incoming call
      vibrate([200, 100, 200]);
    } catch (error) {
      console.error("❌ Error fetching waiting calls:", error?.response?.data || error.message);
    }
  };
  ...
  const acceptCall = async (callId) => {
    try {
      ...
      const response = await axios.put(`${API_BASE_URL}/api/video/calls/${callId}/accept`,
        { acceptorId: userId, acceptorType: 'user' },
  ...
  const rejectCall = async (callId) => {
    try {
      ...
      const response = await axios.post(`${API_BASE_URL}/api/video/calls/${callId}/reject`,
        { userId: userId, userType: 'user' },
        { headers: { Authorization: `Bearer ${token}`, 'Content-Type': 'application/json' } }
      );
  ...
  // Poll for waiting calls every 3 seconds, but only when modal is closed
  useEffect(() => {
    if (!userId || !token) return;
    const poll = () => { if (!showCallModal) fetchWaitingCalls(); };
    poll(); // immediate first check
    const interval = setInterval(poll, 3000);
    return () => clearInterval(interval);
  }, [userId, token, showCallModal]);
  ...
  useEffect(() => {
    const fetchUserData = async () => {
      try {
        ...
        if (!uid) return;
        const response = await axios.get(`${API_BASE_URL}/api/auth/getUser/${uid}`, {
  ...
  const handleLogout = async () => {
    try {
      ...
      await axiosInstance.post(`${API_BASE_URL}/api/auth/logout`, { refreshToken }, {
  ...
  const handleDeleteConfirm = () => {
    ...
    setTimeout(() => navigate('/role-selector'), 2500);
  };
    ...
    <div className="user-dashboard">
      ...
      <CallModal
        ...
        onClose={() => {
          ...
          setCallerInfo({ name: '', image: null, userId: '', userName: '', callId: '', roomId: '' });
        }}
        callType={callType}
      ...
      {isMobile && (
        <div className="mobile-header">
          <div className="mobile-header-left"><h2 className="mobile-logo">MChat</h2></div>
          <div className="mobile-header-right">
            <button className="mobile-profile-btn" onClick={() => setShowProfileMenu(!showProfileMenu)}>
              {userData.profilePhoto ? <img src={userData.profilePhoto} alt={userData.name} className="mobile-user-avatar" onError={(e) => e.target.style.display = 'none'} /> : <FaUserCircle className="mobile-user-icon" />}
            </button>
            {showProfileMenu && (
              <div className="profile-dropdown-menu">
                <div className="profile-dropdown-header">
                  {userData.profilePhoto ? <img src={userData.profilePhoto} alt={userData.name} className="dropdown-avatar" /> : <FaUserCircle className="dropdown-avatar-icon" />}
                  <div className="dropdown-user-info"><h4>{userData.name}</h4><p>{userData.email}</p></div>
                </div>
                <div className="profile-dropdown-items">
                  <button className="dropdown-item" onClick={handleProfileClick}><FaUser className="dropdown-icon" /><span>My Profile</span></button>
                  <button className="dropdown-item logout-item" onClick={handleLogoutClick}><FaSignOutAlt className="dropdown-icon" /><span>Logout</span></button>
                </div>
              </div>
            )}
          </div>
          ...
          <div className="sidebar-content">
            <div className="sidebar-header">
              <div className="profile-section">
                <div className="profile-image">
                  {userData.profilePhoto ? <img src={userData.profilePhoto} alt={userData.name} onError={(e) => e.target.style.display = 'none'} /> : <FaUserCircle className="default-avatar" />}
                </div>
            ...
            <div className="sidebar-menu">
              {allMenuItems.map(item => (
                <button key={item.id} onClick={() => handleMenuItemClick(item.id)} className={`sidebar-item ${active === item.id ? "active" : ""}`}>
          ...
          {active === "help" && (
            <div className="content-section">
              ...
              <div className="help-content">
                <div className="support-card"><FaQuestionCircle className="support-icon" /><h3>FAQ</h3><p>Find answers to frequently asked questions</p></div>
                <div className="support-card"><FaEnvelope className="support-icon" /><h3>Contact Support</h3><p>Email us at support@example.com</p></div>
              </div>
            </div>
          )}
          {active === "privacy" && (
            <div className="content-section">
              ...
              <div className="privacy-content">
                <div className="privacy-option"><h3>Data Privacy</h3><p>Control how your data is used and shared</p><button className="privacy-btn">Manage Settings</button></div>
                <div className="privacy-option"><h3>Session Privacy</h3><p>Configure privacy settings for your sessions</p><button className="privacy-btn">Configure</button></div>
              </div>
            </div>
          )}
        </div>
      ...
      {chatOpen && (
        <ChatPopup messages={chatMessages} newMessage={newMessage} setNewMessage={setNewMessage} sendMessage={sendMessage} handleKeyPress={handleKeyPress} isLoading={isLoading} onClose={() => setChatOpen(false)} chatBodyRef={chatBodyRef} />
      )}
      ...
      {isMobile && (
        <nav className="mobile-bottom-nav">
          {bottomMenuItems.map(item => (
            <button key={item.id} className={`mobile-nav-btn ${active === item.id ? 'active' : ''}`} onClick={() => handleMenuItemClick(item.id)}>
      ...
      {showMoreModal && (
        <div className="modal-overlay" onClick={handleCloseModal}>
          <div className="modal-container more-modal" onClick={e => e.stopPropagation()}>
            <div className="modal-header"><h3 className="modal-title">Menu Options</h3><button className="close-modal-btn" onClick={handleCloseModal}><FaTimes /></button></div>
            <div className="modal-body">
              <div className="more-options-list">
                {allMenuItems.map(item => (
                  <button key={item.id} className={`more-option-item ${active === item.id ? 'active' : ''}`} onClick={() => handleMenuItemClick(item.id)}>
                ...
                <div className="more-actions">
                  <button className="more-action-btn logout-btn" onClick={() => { vibrate(30); setShowMoreModal(false); setShowLogoutConfirm(true); }}>
      ...
      {showLogoutConfirm && (
        <div className="modal-overlay" onClick={() => setShowLogoutConfirm(false)}>
          <div className="modal-container" onClick={e => e.stopPropagation()}>
            <div className="modal-header"><h3 className="modal-title">Confirm Logout</h3></div>
            <div className="modal-body"><p>Are you sure you want to logout?</p></div>
            <div className="modal-footer">
              <button className="btn-secondary" onClick={() => { vibrate(20); setShowLogoutConfirm(false); }}>Cancel</button>
      ...
      {showDeleteConfirm && (
        <div className="modal-overlay" onClick={() => setShowDeleteConfirm(false)}>
          <div className="modal-container" onClick={e => e.stopPropagation()}>
            ...
            <div className="modal-body"><p>This action cannot be undone. All your data will be permanently deleted.</p></div>
            <div className="modal-footer">
              <button className="btn-secondary" onClick={() => { vibrate(20); setShowDeleteConfirm(false); }}>Cancel</button>
      ...
      {deleteSuccess && (
        <div className="modal-overlay">
          <div className="modal-container success-modal">
            <div className="modal-header"><h3 className="modal-title success"><FaCheckCircle className="success-icon" /> Account Deleted!</h3></div>
            <div className="modal-body"><p>Your account has been successfully deleted.</p><p>Redirecting...</p></div>
          </div>
        </div>
      )}
    </div>
