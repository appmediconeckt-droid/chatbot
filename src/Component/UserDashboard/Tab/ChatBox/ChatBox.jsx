      ...
      <div className={`incoming-call-modal ${callType === "video" ? "video-call-modal" : "voice-call-modal"}`}>
        <div className="incoming-call-content">
          <div className="incoming-caller-info">
            ...
            {requestedTime && <p className="incoming-call-time">Received at {requestedTime}</p>}
            <p className="incoming-call-message">{callData?.requestMessage || `Incoming ${callType} call...`}</p>
          </div>
          <div className="incoming-call-controls">
            <button className="incoming-call-btn reject-btn" onClick={handleReject} disabled={isRejecting}>
              {isRejecting ? <FaSpinner className="spinning" /> : <FaPhoneSlash />}
              <span>{isRejecting ? "Rejecting..." : "Decline"}</span>
            </button>
            <button className="incoming-call-btn accept-btn" onClick={handleAccept} disabled={isAccepting}>
...
const ChatBox = () => {
  ...
  const handleRejectCall = async (callId) => {
    try {
      ...
      const userId = resolveCurrentUserId();
      if (!userId) throw new Error("User ID missing. Please login again.");
      await axios.put(`${API_BASE_URL}/api/video/calls/${callId}/reject`, { userId, reason: "declined" }, { headers: { Authorization: `Bearer ${token}` } });
      return true;
    } catch (error) {
      if (error?.response?.status === 404) {
        try {
          const token = localStorage.getItem("token") || localStorage.getItem("accessToken");
          await axios.post(`${API_BASE_URL}/api/call/${callId}/reject`, { reason: "declined" }, { headers: { Authorization: `Bearer ${token}` } });
          return true;
        } catch (fallbackError) {
  ...
  const handleEndCall = async (callId) => {
    try {
      ...
      const userId = resolveCurrentUserId();
      if (!userId) throw new Error("User ID missing. Please login again.");
      await axios.put(`${API_BASE_URL}/api/video/calls/${callId}/end`, { userId: userId, endedBy: "user" }, { headers: { Authorization: `Bearer ${token}` } });
      return true;
    } catch (error) {
  ...
  useEffect(() => {
    const fetchIncomingCalls = async () => {
      try {
        const token = localStorage.getItem("token") || localStorage.getItem("accessToken");
        const userId = resolveCurrentUserId();
        if (!userId || !token || showIncomingModal || isVideoModalOpen) return;
  ...
  useEffect(() => {
    ...
    socket.on("user-typing", ({ userRole, isTyping: typing }) => { if (userRole !== "user") setRemoteIsTyping(typing); });
    socket.on("messages-read", () => { setMessages((prev) => prev.map((msg) => msg.sender === "user" ? { ...msg, isRead: true } : msg)); });
    socket.on("call_rejected", (payload) => { setCallError(`Call was declined${payload?.by ? ` by ${payload.by}` : ""}.`); setIsVideoModalOpen(false); setSelectedCall(null); setShowIncomingModal(false); });
    socket.on("call-status-update", ({ status }) => { if (String(status).toLowerCase() === "rejected") { setCallError("Call was declined by the other participant."); setIsVideoModalOpen(false); setSelectedCall(null); setShowIncomingModal(false); } });
    socket.on("chat-status-update", ({ status, chatId: updatedChatId }) => { setChatStatus(status); setCurrentChat((prev) => (prev ? { ...prev, status } : prev)); });
  ...
  return (
    <div className="chatContainerFull">
      ...
      <VideoCallModal isOpen={isVideoModalOpen} onClose={handleCloseModal} callData={selectedCall} callMode={selectedCall?.callType || selectedCall?.type || "video"} currentUser={currentUser} onEndCall={handleEndCall} />
      <IncomingCallModal isOpen={showIncomingModal} onClose={() => setShowIncomingModal(false)} callType={incomingCallData.callType} callerName={incomingCallData.name} callerImage={incomingCallData.image} callData={incomingCallData} onAcceptCall={handleAcceptCall} onRejectCall={handleRejectCall} />
    </div>
  );
};
