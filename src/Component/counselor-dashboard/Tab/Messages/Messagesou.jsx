...
const SMSList = () => {
      ...
      try {
        const translatedUsers = await Promise.all(
          originalUsers.map(async (user) => {
            ...
            try {
              const translatedMessage = await translateMessage(user.lastMessage, lang);
              return { ...user, lastMessage: translatedMessage, lastMessagePreview: translatedMessage };
            } catch (error) {
              console.error('Error translating user message:', error);
              return user;
            }
    ...
    if (searchTerm) {
      filtered = filtered.filter(
        (user) =>
          user.name.toLowerCase().includes(searchTerm.toLowerCase()) ||
          user.specialization.toLowerCase().includes(searchTerm.toLowerCase()) ||
          user.condition.toLowerCase().includes(searchTerm.toLowerCase())
      );
    }
  ...
  const getPriorityClass = (priority) => {
    ...
    if (priority === "follow-up") return "priority-followup";
    ...
    <div className={`counselor-chat-workspace ${selectedConversation ? "has-open-chat" : ""}`}>
      ...
      {/* <div className="smslist-section">
        ...
        {filteredUsers.slice(0, 2).map((user) => (
          <div
            key={user.id}
            className={`smslist-user-item pinned-item ${selectedChatId === user.chatId ? "selected" : ""}`}
            onClick={() => handleUserClick(user)}
          >
            <div className="smslist-user-avatar">
              {user.avatarUrl ? (
            ...
            <div className="smslist-user-info">
              ...
              <div className="smslist-last-message">
                <p className="message-preview">{user.lastMessagePreview}</p>
                {user.unread > 0 && (
                  <span className="unread-count">{user.unread}</span>
                )}
              </div>
        ...
        <div className="smslist-pinned-row">
          {users.slice(0, 4).map((user) => (
            <button type="button" key={`pinned-${user.id}`} onClick={() => handleUserClick(user)}>
              <span className="smslist-pinned-avatar">
                {user.avatarUrl ? <img src={user.avatarUrl} alt="" /> : <span>{user.avatar || getInitials(user.name)}</span>}
                <i className={user.online ? "online" : "offline"} />
              </span>
              <small>{String(user.name || "Patient").split(" ")[0]}</small>
            </button>
          ))}
        </div>
      </div> */}
      ...
      <div className="smslist-users">
        {filteredUsers.length > 0 ? (
          filteredUsers.map((user) => (
            <div
              key={user.id}
              className={`smslist-user-item ${selectedChatId === user.chatId ? "selected" : ""} ${getPriorityClass(user.priority)}`}
              onClick={() => handleUserClick(user)}
              onContextMenu={(event) => {
                event.preventDefault();
                event.stopPropagation();
            ...
            >
              ...
              <div className="smslist-user-info">
                ...
                <div className="smslist-user-details">
                  <span className={`priority-badge ${getPriorityClass(user.priority)}`}>
                ...
                <div className="smslist-user-message-preview">
                  <span className="last-message-text">{user.lastMessagePreview}</span>
                </div>
              </div>
            </div>
          ))
      ...
      {selectedConversation && (
        <div className="counselor-chat-open-pane">
          <SMSInput
            embeddedUser={selectedConversation}
            embeddedChatId={selectedConversation.chatId}
            onEmbeddedBack={() => {
