      ...
      <div className={`incoming-call-modal ${callType === "video" ? "video-call-modal" : "voice-call-modal"}`}>
        <div className="incoming-call-content">
          ...
          <div className="incoming-call-controls">
            ...
            <button className="incoming-call-btn accept-btn" onClick={handleJoin} disabled={isJoining}>
...
const SMSInput = () => {
    ...
    try {
      ...
      if (response.data && response.data.success) {
        const callDataForModal = {
          ...
          callId: callId,
          roomId: response.data.roomId || incomingCallData.roomId,
  ...
  const handleRejectIncomingCall = async (callId) => {
    try {
      const token = localStorage.getItem("token") || localStorage.getItem("accessToken");
      await axios.put(`${API_BASE_URL}/api/video/calls/${callId}/reject`, { userId: COUNSELOR_ID, reason: "declined" }, { headers: { Authorization: `Bearer ${token}` } });
      return true;
  ...
  const handleEndIncomingCall = async (callId) => {
    try {
      const token = localStorage.getItem("token") || localStorage.getItem("accessToken");
      await axios.put(`${API_BASE_URL}/api/video/calls/${callId}/end`, { userId: COUNSELOR_ID, endedBy: "counsellor" }, { headers: { Authorization: `Bearer ${token}` } });
      return true;
  ...
  useEffect(() => {
    ...
    const fetchIncomingCalls = async () => {
      try {
        const token = localStorage.getItem("token") || localStorage.getItem("accessToken");
        if (!COUNSELOR_ID || !token || showIncomingModal || isVideoModalOpen) return;
  ...
  useEffect(() => {
    ...
    socket.on("user-typing", ({ userRole, isTyping }) => { if (userRole === "user") setRemoteIsTyping(isTyping); });
    socket.on("messages-read", () => { setMessages((prev) => prev.map((msg) => msg.sender === "me" ? { ...msg, isRead: true } : msg)); });
    ...
    socket.on("call-status-update", ({ status }) => { if (String(status).toLowerCase() === "rejected") { setCallError("Call was declined."); setIsVideoModalOpen(false); setSelectedCall(null); setShowIncomingModal(false); } });
    return () => { if (chatSocketRef.current) { chatSocketRef.current.disconnect(); chatSocketRef.current = null; } };
  ...
  return (
    <div className="smsinput-container">
      ...
      {/* Call Modals */}
      <VideoCallModal isOpen={isVideoModalOpen} onClose={handleCloseModal} callData={selectedCall} callMode={selectedCall?.callType || selectedCall?.type || "video"} currentUser={{ id: COUNSELOR_ID, role: "counsellor" }} onEndCall={handleEndIncomingCall} />
      <IncomingCallModal isOpen={showIncomingModal} onClose={() => setShowIncomingModal(false)} callType={incomingCallData.callType} callerName={incomingCallData.name} callerAvatar={incomingCallData.avatar} callData={incomingCallData} onJoinCall={handleJoinIncomingCall} onRejectCall={handleRejectIncomingCall} />
    </div>
  );
};
